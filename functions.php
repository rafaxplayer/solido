...
if ( ! function_exists( 'solido_setup' ) ) :
	...
	function solido_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'solido' ),
		) );
...
function solido_scripts() {
	...
	wp_enqueue_style( 'solido-font-awesome', $directory_uri . '/assets/css/fontawesome-all.css' );
	
	wp_enqueue_style( 'solido-style', get_stylesheet_uri() );
	
	wp_enqueue_script( 'solido-navigation', $directory_uri . '/assets/js/navigation.js', array(), '20151215', true );
...
// Custom excerpt more
function solido_excerpt_more($link){

	if ( is_admin() ) {
        return $link;
    }
    $link = sprintf( '<div class="solido-button"><a href="%1$s">%2$s</a></div>',
...
//add_filter('excerpt_more', 'solido_excerpt_more');
